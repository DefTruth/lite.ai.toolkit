# How to convert DeepLabV3ResNet101 to ONNX

## 1. 前言

这篇文档主要记录将项目[DeepLabv3ResNet101](https://pytorch.org/hub/pytorch_vision_deeplabv3_resnet101) 的模型转换成onnx模型.
...
```python
def convert_dynamic_deeplabv3_onnx():
    import numpy as np
    onnx_path = "./weights/deeplabv3_resnet101_coco.onnx"
    sim_onnx_path = "./weights/deeplabv3_resnet101_coco-sim.onnx"
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    model = torch.hub.load('pytorch/vision:v0.9.0', 'deeplabv3_resnet101', pretrained=True).to(device)
    model.eval()

    # x = torch.randn(1, 3, 512, 512).cpu()  # moving the tensor to cpu
