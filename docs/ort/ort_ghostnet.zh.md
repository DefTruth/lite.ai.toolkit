# How to convert GhostNet to ONNX

## 1. 前言

这篇文档主要记录将项目[GhostNet](https://pytorch.org/hub/pytorch_vision_ghostnet) 的模型转换成onnx模型.
...
def convert_static_ghostnet():
    onnx_path = "./weights/ghostnet.onnx"
    sim_onnx_path = "./weights/ghostnet-sim.onnx"
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    model = torch.hub.load('huawei-noah/ghostnet', 'ghostnet_1x', pretrained=True).to(device)
...
if __name__ == "__main__":
    convert_static_ghostnet()

    """
    PYTHONPATH=. python3 ./ghostnet.py
    """
