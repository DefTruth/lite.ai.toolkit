# How to convert HarDNet to ONNX

## 1. 前言

这篇文档主要记录将项目[HarDNet](https://pytorch.org/hub/pytorch_vision_hardnet) 的模型转换成onnx模型.
...
def convert_static_hardnet():
    import numpy as np

    onnx_path = "./weights/hardnet.onnx"
    sim_onnx_path = "./weights/hardnet-sim.onnx"
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    model = torch.hub.load('PingoLH/Pytorch-HarDNet', 'hardnet39ds', pretrained=True).to(device)
...
if __name__ == "__main__":
    convert_static_hardnet()

    """
    PYTHONPATH=. python3 ./hardnet.py
    """
```
