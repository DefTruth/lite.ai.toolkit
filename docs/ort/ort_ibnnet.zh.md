# How to convert IBNNet to ONNX

## 1. 前言

这篇文档主要记录将项目[IBNNet](https://pytorch.org/hub/pytorch_vision_ibnnet) 的模型转换成onnx模型.
...
def convert_static_ibnnet():
    import numpy as np

    onnx_path = "./weights/ibnnet18.onnx"
    sim_onnx_path = "./weights/ibnnet18-sim.onnx"
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model = torch.hub.load('XingangPan/IBN-Net', 'resnet18_ibn_a', pretrained=True).to(device)
    model.eval()
...
if __name__ == "__main__":
    convert_static_ibnnet()

    """
    PYTHONPATH=. python3 ./ibnnet.py
    """
