# How to convert ResNeXt to ONNX

## 1. 前言

这篇文档主要记录将项目[ResNeXt](https://pytorch.org/hub/pytorch_vision_resnext) 的模型转换成onnx模型.
...
def convert_static_resnext():
    import numpy as np

    onnx_path = "./weights/resnext50.onnx"
    sim_onnx_path = "./weights/resnext50-sim.onnx"
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    model = torch.hub.load('pytorch/vision:v0.9.0', 'resnext50_32x4d', pretrained=True).to(device)
...
if __name__ == "__main__":
    convert_static_resnext()

    """
    PYTHONPATH=. python3 ./resnext.py
    """
