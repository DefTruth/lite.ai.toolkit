...
static void test_default()
{
  ...
  lite::cv::detection::EfficientDet *efficientdet =
      new lite::cv::detection::EfficientDet(onnx_path, 8); // default

  std::vector<lite::cv::types::Boxf> detected_boxes;
  cv::Mat img_bgr = cv::imread(test_img_path);
  efficientdet->detect(img_bgr, detected_boxes);
...
static void test_onnxruntime()
{
  ...
  lite::onnxruntime::cv::detection::EfficientDet *efficientdet =
      new lite::onnxruntime::cv::detection::EfficientDet(onnx_path, 8);
