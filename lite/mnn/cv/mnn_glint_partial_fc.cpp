...
#include "mnn_glint_partial_fc.h"

using mnncv::MNNGlintPartialFC;

MNNGlintPartialFC::MNNGlintPartialFC(const std::string &_mnn_path, unsigned int _num_threads) :
...
inline void MNNGlintPartialFC::initialize_pretreat()
{
...
void MNNGlintPartialFC::transform(const cv::Mat &mat)
{
...
void MNNGlintPartialFC::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  this->transform(mat);
