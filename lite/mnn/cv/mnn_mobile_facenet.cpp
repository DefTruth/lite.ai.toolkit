...
#include "mnn_mobile_facenet.h"

using mnncv::MNNMobileFaceNet;

MNNMobileFaceNet::MNNMobileFaceNet(const std::string &_mnn_path, unsigned int _num_threads) :
...
inline void MNNMobileFaceNet::initialize_pretreat()
{
...
void MNNMobileFaceNet::transform(const cv::Mat &mat)
{
...
void MNNMobileFaceNet::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  this->transform(mat);
