...
#include "mnn_mobile_hair_seg.h"
#include "lite/utils.h"

using mnncv::MNNMobileHairSeg;

MNNMobileHairSeg::MNNMobileHairSeg(const std::string &_mnn_path, unsigned int _num_threads)
    : BasicMNNHandler(_mnn_path, _num_threads)
{
  initialize_pretreat();
}

void MNNMobileHairSeg::initialize_pretreat()
{
...
void MNNMobileHairSeg::transform(const cv::Mat &mat)
{
  cv::Mat canvas;
...
void MNNMobileHairSeg::detect(const cv::Mat &mat, types::HairSegContent &content,
                                     ...
                                     float score_threshold, bool remove_noise)
{
  ...
  float *output_ptr = host_output_tensor.host<float>() + element_size; // only need positive prob
