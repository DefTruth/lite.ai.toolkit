...
#include "mnn_mobilenetv2_se_68.h"

using mnncv::MNNMobileNetV2SE68;

MNNMobileNetV2SE68::MNNMobileNetV2SE68(const std::string &_mnn_path, unsigned int _num_threads)
    : BasicMNNHandler(_mnn_path, _num_threads)
{
  initialize_pretreat();
}

inline void MNNMobileNetV2SE68::initialize_pretreat()
{
...
void MNNMobileNetV2SE68::transform(const cv::Mat &mat)
{
  cv::Mat mat_rs;
...
void MNNMobileNetV2SE68::detect(const cv::Mat &mat, types::Landmarks &landmarks)
{
  if (mat.empty()) return;
  // this->transform(mat);
