...
#include "mnn_mobilese_focal_face.h"

using mnncv::MNNMobileSEFocalFace;

MNNMobileSEFocalFace::MNNMobileSEFocalFace(const std::string &_mnn_path, unsigned int _num_threads) :
    BasicMNNHandler(_mnn_path, _num_threads)
{
  initialize_pretreat();
}

inline void MNNMobileSEFocalFace::initialize_pretreat()
{
...
void MNNMobileSEFocalFace::transform(const cv::Mat &mat)
{
...
void MNNMobileSEFocalFace::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  this->transform(mat);
