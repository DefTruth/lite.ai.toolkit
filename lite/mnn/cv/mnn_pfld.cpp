...
MNNPFLD::MNNPFLD(const std::string &_mnn_path, unsigned int _num_threads)
    : BasicMNNHandler(_mnn_path, _num_threads)
{
  initialize_pretreat();
}

inline void MNNPFLD::initialize_pretreat()
{
...
void MNNPFLD::transform(const cv::Mat &mat)
{
  cv::Mat mat_rs;
...
void MNNPFLD::detect(const cv::Mat &mat, types::Landmarks &landmarks)
{
  if (mat.empty()) return;
  // this->transform(mat);
  ...
  // 3. fetch landmarks.
  auto device_landmarks_ptr = output_tensors.at("output"); // (1,212)
