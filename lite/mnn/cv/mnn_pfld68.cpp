...
#include "mnn_pfld68.h"

using mnncv::MNNPFLD68;

MNNPFLD68::MNNPFLD68(const std::string &_mnn_path, unsigned int _num_threads)
    : BasicMNNHandler(_mnn_path, _num_threads)
{
  initialize_pretreat();
}

inline void MNNPFLD68::initialize_pretreat()
{
...
void MNNPFLD68::transform(const cv::Mat &mat)
{
  cv::Mat mat_rs;
...
void MNNPFLD68::detect(const cv::Mat &mat, types::Landmarks &landmarks)
{
  if (mat.empty()) return;
  // this->transform(mat);
