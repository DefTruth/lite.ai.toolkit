...
#include "mnn_pipnet19.h"

using mnncv::MNNPIPNet19;

MNNPIPNet19::MNNPIPNet19(const std::string &_mnn_path, unsigned int _num_threads)
    : BasicMNNHandler(_mnn_path, _num_threads)
{
  initialize_pretreat();
}

inline void MNNPIPNet19::initialize_pretreat()
{
...
void MNNPIPNet19::transform(const cv::Mat &mat)
{
  cv::Mat canvas;
...
void MNNPIPNet19::detect(const cv::Mat &mat, types::Landmarks &landmarks)
{
  if (mat.empty()) return;
  // this->transform(mat);
...
void MNNPIPNet19::generate_landmarks(types::Landmarks &landmarks,
                                     const std::map<std::string, MNN::Tensor *> &output_tensors,
                                     float img_height, float img_width)
{
  auto device_outputs_cls_ptr = output_tensors.at("outputs_cls"); // (1,19,8,8)
  auto device_outputs_x_ptr = output_tensors.at("outputs_x"); // (1,19,8,8)
  auto device_outputs_y_ptr = output_tensors.at("outputs_y"); // (1,19,8,8)
  auto device_outputs_nb_x_ptr = output_tensors.at("outputs_nb_x"); // (1,19*10,8,8)
  auto device_outputs_nb_y_ptr = output_tensors.at("outputs_nb_y"); // (1,19*10,8,8)
  ...
  // calculate coords
  std::vector<float> lms_pred_x(num_lms); // 19
  std::vector<float> lms_pred_y(num_lms); // 19
  ...
  // reverse indexes
  std::unordered_map<unsigned int, std::vector<float>> tmp_nb_x; // 19,max_len
  std::unordered_map<unsigned int, std::vector<float>> tmp_nb_y; // 19,max_len
