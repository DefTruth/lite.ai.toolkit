...
#include "mnn_pipnet29.h"

using mnncv::MNNPIPNet29;

MNNPIPNet29::MNNPIPNet29(const std::string &_mnn_path, unsigned int _num_threads)
    : BasicMNNHandler(_mnn_path, _num_threads)
{
  initialize_pretreat();
}

inline void MNNPIPNet29::initialize_pretreat()
{
...
void MNNPIPNet29::transform(const cv::Mat &mat)
{
  cv::Mat canvas;
...
void MNNPIPNet29::detect(const cv::Mat &mat, types::Landmarks &landmarks)
{
  if (mat.empty()) return;
  // this->transform(mat);
...
void MNNPIPNet29::generate_landmarks(types::Landmarks &landmarks,
                                     const std::map<std::string, MNN::Tensor *> &output_tensors,
                                     float img_height, float img_width)
{
  auto device_outputs_cls_ptr = output_tensors.at("outputs_cls"); // (1,29,8,8)
  auto device_outputs_x_ptr = output_tensors.at("outputs_x"); // (1,29,8,8)
  auto device_outputs_y_ptr = output_tensors.at("outputs_y"); // (1,29,8,8)
  auto device_outputs_nb_x_ptr = output_tensors.at("outputs_nb_x"); // (1,29*10,8,8)
  auto device_outputs_nb_y_ptr = output_tensors.at("outputs_nb_y"); // (1,29*10,8,8)
  ...
  // calculate coords
  std::vector<float> lms_pred_x(num_lms); // 29
  std::vector<float> lms_pred_y(num_lms); // 29
  std::unordered_map<unsigned int, std::vector<float>> lms_pred_nb_x; // 29,10
  std::unordered_map<unsigned int, std::vector<float>> lms_pred_nb_y; // 29,10
  ...
  // reverse indexes
  std::unordered_map<unsigned int, std::vector<float>> tmp_nb_x; // 29,max_len
  std::unordered_map<unsigned int, std::vector<float>> tmp_nb_y; // 29,max_len
