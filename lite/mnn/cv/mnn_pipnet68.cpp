...
#include "mnn_pipnet68.h"

using mnncv::MNNPIPNet68;

MNNPIPNet68::MNNPIPNet68(const std::string &_mnn_path, unsigned int _num_threads)
    : BasicMNNHandler(_mnn_path, _num_threads)
{
  initialize_pretreat();
}

inline void MNNPIPNet68::initialize_pretreat()
{
...
void MNNPIPNet68::transform(const cv::Mat &mat)
{
  cv::Mat canvas;
...
void MNNPIPNet68::detect(const cv::Mat &mat, types::Landmarks &landmarks)
{
  if (mat.empty()) return;
  // this->transform(mat);
...
void MNNPIPNet68::generate_landmarks(types::Landmarks &landmarks,
                                     const std::map<std::string, MNN::Tensor *> &output_tensors,
                                     float img_height, float img_width)
{
  auto device_outputs_cls_ptr = output_tensors.at("outputs_cls"); // (1,68,8,8)
  auto device_outputs_x_ptr = output_tensors.at("outputs_x"); // (1,68,8,8)
  auto device_outputs_y_ptr = output_tensors.at("outputs_y"); // (1,68,8,8)
  auto device_outputs_nb_x_ptr = output_tensors.at("outputs_nb_x"); // (1,68*10,8,8)
  auto device_outputs_nb_y_ptr = output_tensors.at("outputs_nb_y"); // (1,68*10,8,8)
