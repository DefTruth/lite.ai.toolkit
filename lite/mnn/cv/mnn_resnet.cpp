...
#include "mnn_resnet.h"
#include "lite/utils.h"

using mnncv::MNNResNet;

MNNResNet::MNNResNet(const std::string &_mnn_path, unsigned int _num_threads)
    : BasicMNNHandler(_mnn_path, _num_threads)
{
  initialize_pretreat();
}

inline void MNNResNet::initialize_pretreat()
{
...
void MNNResNet::transform(const cv::Mat &mat)
{
  cv::Mat canvas;
...
void MNNResNet::detect(const cv::Mat &mat, types::ImageNetContent &content, unsigned int top_k)
{
  if (mat.empty()) return;
  // 1. make input tensor
  this->transform(mat);
