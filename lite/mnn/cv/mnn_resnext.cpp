...
#include "mnn_resnext.h"
#include "lite/utils.h"

using mnncv::MNNResNeXt;

MNNResNeXt::MNNResNeXt(const std::string &_mnn_path, unsigned int _num_threads)
    : BasicMNNHandler(_mnn_path, _num_threads)
{
  initialize_pretreat();
}

inline void MNNResNeXt::initialize_pretreat()
{
...
void MNNResNeXt::transform(const cv::Mat &mat)
{
  cv::Mat canvas;
...
void MNNResNeXt::detect(const cv::Mat &mat, types::ImageNetContent &content, unsigned int top_k)
{
  if (mat.empty()) return;
  // 1. make input tensor
  this->transform(mat);
