...
#include "mnn_rexnet_emotion7.h"
#include "lite/utils.h"

using mnncv::MNNReXNetEmotion7;

MNNReXNetEmotion7::MNNReXNetEmotion7(const std::string &_mnn_path, unsigned int _num_threads)
    : BasicMNNHandler(_mnn_path, _num_threads)
{
  initialize_pretreat();
}

inline void MNNReXNetEmotion7::initialize_pretreat()
{
...
void MNNReXNetEmotion7::transform(const cv::Mat &mat)
{
  cv::Mat canvas;
...
void MNNReXNetEmotion7::detect(const cv::Mat &mat, types::Emotions &emotions)
{
  if (mat.empty()) return;
  // 1. make input tensor
  this->transform(mat);
