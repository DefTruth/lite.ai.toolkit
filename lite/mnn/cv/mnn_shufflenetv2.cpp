...
#include "mnn_shufflenetv2.h"
#include "lite/utils.h"

using mnncv::MNNShuffleNetV2;

MNNShuffleNetV2::MNNShuffleNetV2(const std::string &_mnn_path, unsigned int _num_threads)
    : BasicMNNHandler(_mnn_path, _num_threads)
{
  initialize_pretreat();
}

inline void MNNShuffleNetV2::initialize_pretreat()
{
...
void MNNShuffleNetV2::transform(const cv::Mat &mat)
{
  cv::Mat canvas;
...
void MNNShuffleNetV2::detect(const cv::Mat &mat, types::ImageNetContent &content, unsigned int top_k)
{
  if (mat.empty()) return;
  // 1. make input tensor
  this->transform(mat);
  ...
  // 3. fetch.
  auto device_logits_ptr = output_tensors.at("output");
