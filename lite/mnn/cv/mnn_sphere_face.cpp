...
#include "mnn_sphere_face.h"

using mnncv::MNNSphereFace;

MNNSphereFace::MNNSphereFace(const std::string &_mnn_path, unsigned int _num_threads) :
...
inline void MNNSphereFace::initialize_pretreat()
{
...
void MNNSphereFace::transform(const cv::Mat &mat)
{
...
void MNNSphereFace::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  this->transform(mat);
