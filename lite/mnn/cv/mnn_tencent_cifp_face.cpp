...
#include "mnn_tencent_cifp_face.h"

using mnncv::MNNTencentCifpFace;

MNNTencentCifpFace::MNNTencentCifpFace(const std::string &_mnn_path, unsigned int _num_threads) :
...
inline void MNNTencentCifpFace::initialize_pretreat()
{
...
void MNNTencentCifpFace::transform(const cv::Mat &mat)
{
...
void MNNTencentCifpFace::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  this->transform(mat);
