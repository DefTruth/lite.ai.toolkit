...
#include "mnn_tencent_curricular_face.h"

using mnncv::MNNTencentCurricularFace;

MNNTencentCurricularFace::MNNTencentCurricularFace(const std::string &_mnn_path, unsigned int _num_threads) :
...
inline void MNNTencentCurricularFace::initialize_pretreat()
{
...
void MNNTencentCurricularFace::transform(const cv::Mat &mat)
{
...
void MNNTencentCurricularFace::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  this->transform(mat);
