...
#include "ncnn_facenet.h"

using ncnncv::NCNNFaceNet;

void NCNNFaceNet::transform(const cv::Mat &mat, ncnn::Mat &in)
{
  // BGR NHWC -> RGB NCHW
  int h = mat.rows;
  int w = mat.cols;
...
void NCNNFaceNet::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  ncnn::Mat input;
