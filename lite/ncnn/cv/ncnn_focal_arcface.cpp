...
#include "ncnn_focal_arcface.h"

using ncnncv::NCNNFocalArcFace;

void NCNNFocalArcFace::transform(const cv::Mat &mat, ncnn::Mat &in)
{
  // BGR NHWC -> RGB NCHW
  int h = mat.rows;
  int w = mat.cols;
...
void NCNNFocalArcFace::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  ncnn::Mat input;
