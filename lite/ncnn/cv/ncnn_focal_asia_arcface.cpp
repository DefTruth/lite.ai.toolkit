...
#include "ncnn_focal_asia_arcface.h"

using ncnncv::NCNNFocalAsiaArcFace;

void NCNNFocalAsiaArcFace::transform(const cv::Mat &mat, ncnn::Mat &in)
{
  // BGR NHWC -> RGB NCHW
  int h = mat.rows;
  int w = mat.cols;
...
void NCNNFocalAsiaArcFace::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  ncnn::Mat input;
