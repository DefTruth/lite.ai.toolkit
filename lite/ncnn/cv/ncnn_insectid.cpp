...
#include "ncnn_insectid.h"
#include "lite/utils.h"

using ncnncv::NCNNInsectID;

NCNNInsectID::NCNNInsectID(const std::string &_param_path,
...
void NCNNInsectID::transform(const cv::Mat &mat, ncnn::Mat &in)
{
  cv::Mat mat_rs;
...
void NCNNInsectID::detect(const cv::Mat &mat, types::ImageNetContent &content, unsigned int top_k)
{
  if (mat.empty()) return;
  ...
  // 3. fetch.
  ncnn::Mat logits_mat;
  extractor.extract("477", logits_mat); // c=1,h=1,w=2037
#ifdef LITENCNN_DEBUG
  BasicNCNNHandler::print_shape(logits_mat, "477");
#endif
