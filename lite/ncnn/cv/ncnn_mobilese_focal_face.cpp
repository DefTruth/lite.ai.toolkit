...
#include "ncnn_mobilese_focal_face.h"

using ncnncv::NCNNMobileSEFocalFace;

void NCNNMobileSEFocalFace::transform(const cv::Mat &mat, ncnn::Mat &in)
{
  // BGR NHWC -> RGB NCHW
  int h = mat.rows;
  int w = mat.cols;
...
void NCNNMobileSEFocalFace::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  ncnn::Mat input;
  ...
  const unsigned int hidden_dim = embedding.c; // 256
