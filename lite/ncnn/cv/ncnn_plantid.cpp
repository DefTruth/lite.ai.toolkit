...
#include "ncnn_plantid.h"
#include "lite/utils.h"

using ncnncv::NCNNPlantID;

NCNNPlantID::NCNNPlantID(const std::string &_param_path,
...
void NCNNPlantID::transform(const cv::Mat &mat, ncnn::Mat &in)
{
  cv::Mat mat_rs;
...
void NCNNPlantID::detect(const cv::Mat &mat, types::ImageNetContent &content, unsigned int top_k)
{
  if (mat.empty()) return;
  ...
  // 3. fetch.
  ncnn::Mat logits_mat;
  extractor.extract("477", logits_mat); // c=1,h=1,w=4066
