...
#include "ncnn_shufflenetv2.h"
#include "lite/utils.h"

using ncnncv::NCNNShuffleNetV2;

NCNNShuffleNetV2::NCNNShuffleNetV2(const std::string &_param_path,
...
void NCNNShuffleNetV2::transform(const cv::Mat &mat, ncnn::Mat &in)
{
  cv::Mat mat_rs;
...
void NCNNShuffleNetV2::detect(const cv::Mat &mat, types::ImageNetContent &content, unsigned int top_k)
{
  if (mat.empty()) return;
  ...
  // 3. fetch.
  ncnn::Mat logits_mat;
  extractor.extract("output", logits_mat); // c=1,h=1,w=1000
#ifdef LITENCNN_DEBUG
  BasicNCNNHandler::print_shape(logits_mat, "output");
#endif
