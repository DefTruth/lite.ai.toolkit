...
Ort::Value FaceNet::transform(const cv::Mat &mat)
{
...
void FaceNet::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  ...
  // the output values of facenet were normalized.
