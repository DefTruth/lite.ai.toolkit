...
using ortcv::InsectID;

Ort::Value InsectID::transform(const cv::Mat &mat)
{
  cv::Mat canvas;
...
void InsectID::detect(const cv::Mat &mat, types::ImageNetContent &content, unsigned int top_k)
{
  if (mat.empty()) return;
  // 1. make input tensor
  ...
  Ort::Value &logits_tensor = output_tensors.at(0); // (1,2037)
