...
using ortcv::MobileFaceNet;

Ort::Value MobileFaceNet::transform(const cv::Mat &mat)
{
...
void MobileFaceNet::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  ...
  auto embedding_dims = output_node_dims.at(0); // (1,128)
  const unsigned int hidden_dim = embedding_dims.at(1); // 128
