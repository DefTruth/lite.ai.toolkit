...
using ortcv::PIPNet29;

Ort::Value PIPNet29::transform(const cv::Mat &mat)
{
  cv::Mat canvas;
...
void PIPNet29::detect(const cv::Mat &mat, types::Landmarks &landmarks)
{
  if (mat.empty()) return;
...
void PIPNet29::generate_landmarks(types::Landmarks &landmarks,
                                  std::vector<Ort::Value> &output_tensors,
                                  float img_height, float img_width)
{
  Ort::Value &outputs_cls = output_tensors.at(0);  // (1,29,8,8)
  Ort::Value &outputs_x = output_tensors.at(1);  // (1,29,8,8)
  Ort::Value &outputs_y = output_tensors.at(2);  // (1,29,8,8)
  Ort::Value &outputs_nb_x = output_tensors.at(3);  // (1,29*10,8,8)
  Ort::Value &outputs_nb_y = output_tensors.at(4);  // (1,29*10,8,8)
