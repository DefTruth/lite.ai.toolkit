...
void TinyYoloV3::print_debug_string()
{
...
void TinyYoloV3::detect(const cv::Mat &mat, std::vector<types::Boxf> &detected_boxes)
{
  if (mat.empty()) return;
   ...
   * indices: (1x'nbox'x3) selected indices from the boxes tensor after NMS.
   * note that the output shape of 'indices' is slight different with 'YoloV3'
   * which the output shape of 'indices'(yolov3) is ('nbox'x3)
                                 ...
                                 std::vector<Ort::Value> &output_tensors)
{
  ...
  {
    unsigned int batch_index = indices.At<unsigned int>({0, i, 0});
    unsigned int class_index = indices.At<unsigned int>({0, i, 1});
    unsigned int box_index = indices.At<unsigned int>({0, i, 2});
