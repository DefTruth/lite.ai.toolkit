...
#include "tiny_yolov4_coco.h"
#include "lite/ort/core/ort_utils.h"
#include "lite/utils.h"

using ortcv::TinyYoloV4COCO;

Ort::Value TinyYoloV4COCO::transform(const cv::Mat &mat)
{
...
void TinyYoloV4COCO::detect(const cv::Mat &mat, std::vector<types::Boxf> &detected_boxes,
...
void TinyYoloV4COCO::generate_bboxes(std::vector<types::Boxf> &bbox_collection,
                                     std::vector<Ort::Value> &output_tensors,
                                     float score_threshold, float img_height,
                                     float img_width)
{
  Ort::Value &pred = output_tensors.at(0); // (1,n,25=5+20=cxcy+cwch+obj_conf+cls_conf)
  auto pred_dims = output_node_dims.at(0); // (1,n,25)
  const unsigned int num_anchors = pred_dims.at(1); // n = ?
  const unsigned int num_classes = pred_dims.at(2) - 5; // 20
  const float input_height = static_cast<float>(input_node_dims.at(2)); // e.g 416
  const float input_width = static_cast<float>(input_node_dims.at(3)); // e.g 416
...
void TinyYoloV4COCO::nms(std::vector<types::Boxf> &input, std::vector<types::Boxf> &output,
