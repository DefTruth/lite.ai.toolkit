...
#include "tnn_cava_ghost_arcface.h"

using tnncv::TNNCavaGhostArcFace;

TNNCavaGhostArcFace::TNNCavaGhostArcFace(const std::string &_proto_path,
...
void TNNCavaGhostArcFace::transform(const cv::Mat &mat_rs)
{
  // cv::Mat canvas;
...
void TNNCavaGhostArcFace::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  cv::Mat mat_rs;
