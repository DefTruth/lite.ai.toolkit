...
#include "tnn_efficientnet_lite4.h"
#include "lite/utils.h"

using tnncv::TNNEfficientNetLite4;

TNNEfficientNetLite4::TNNEfficientNetLite4(const std::string &_proto_path,
...
void TNNEfficientNetLite4::transform(const cv::Mat &mat_rs)
{
...
void TNNEfficientNetLite4::detect(const cv::Mat &mat, types::ImageNetContent &content, unsigned int top_k)
{
  if (mat.empty()) return;
  ...
  std::shared_ptr<tnn::Mat> scores_mat; // (1,1000)
  status = instance->GetOutputMat(scores_mat, cvt_param, "Softmax:0", output_device_type);
  ...
  const float *scores = (float *) scores_mat->GetData();
