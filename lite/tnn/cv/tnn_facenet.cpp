...
#include "tnn_facenet.h"

using tnncv::TNNFaceNet;

TNNFaceNet::TNNFaceNet(const std::string &_proto_path,
...
void TNNFaceNet::transform(const cv::Mat &mat_rs)
{
  // push into input_mat
...
void TNNFaceNet::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  cv::Mat mat_rs;
