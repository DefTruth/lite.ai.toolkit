...
#include "tnn_focal_asia_arcface.h"

using tnncv::TNNFocalAsiaArcFace;

TNNFocalAsiaArcFace::TNNFocalAsiaArcFace(const std::string &_proto_path,
...
void TNNFocalAsiaArcFace::transform(const cv::Mat &mat_rs)
{
  // push into input_mat
...
void TNNFocalAsiaArcFace::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  cv::Mat mat_rs;
