...
#include "tnn_glint_cosface.h"

using tnncv::TNNGlintCosFace;

TNNGlintCosFace::TNNGlintCosFace(const std::string &_proto_path,
...
void TNNGlintCosFace::transform(const cv::Mat &mat_rs)
{
  // push into input_mat
...
void TNNGlintCosFace::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  cv::Mat mat_rs;
