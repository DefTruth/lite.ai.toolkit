...
#include "tnn_glint_partial_fc.h"

using tnncv::TNNGlintPartialFC;

TNNGlintPartialFC::TNNGlintPartialFC(const std::string &_proto_path,
...
void TNNGlintPartialFC::transform(const cv::Mat &mat_rs)
{
  // push into input_mat
...
void TNNGlintPartialFC::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  cv::Mat mat_rs;
