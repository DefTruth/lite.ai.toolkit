...
#include "tnn_mobile_facenet.h"

using tnncv::TNNMobileFaceNet;

TNNMobileFaceNet::TNNMobileFaceNet(const std::string &_proto_path,
...
void TNNMobileFaceNet::transform(const cv::Mat &mat_rs)
{
  // push into input_mat
...
void TNNMobileFaceNet::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  cv::Mat mat_rs;
