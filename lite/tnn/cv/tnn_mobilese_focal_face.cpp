...
#include "tnn_mobilese_focal_face.h"

using tnncv::TNNMobileSEFocalFace;

TNNMobileSEFocalFace::TNNMobileSEFocalFace(const std::string &_proto_path,
...
void TNNMobileSEFocalFace::transform(const cv::Mat &mat_rs)
{
  // push into input_mat
...
void TNNMobileSEFocalFace::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  cv::Mat mat_rs;
