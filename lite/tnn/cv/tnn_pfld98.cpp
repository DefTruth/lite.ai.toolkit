...
#include "tnn_pfld98.h"

using tnncv::TNNPFLD98;

TNNPFLD98::TNNPFLD98(const std::string &_proto_path,
...
void TNNPFLD98::transform(const cv::Mat &mat_rs)
{
  // push into input_mat
...
void TNNPFLD98::detect(const cv::Mat &mat, types::Landmarks &landmarks)
{
  if (mat.empty()) return;
  ...
  std::shared_ptr<tnn::Mat> landmarks_norm; // // (1,98*2)
  status = instance->GetOutputMat(landmarks_norm, cvt_param, "landmarks", output_device_type);
