...
#include "tnn_plantid.h"
#include "lite/utils.h"

using tnncv::TNNPlantID;

TNNPlantID::TNNPlantID(const std::string &_proto_path,
...
void TNNPlantID::transform(const cv::Mat &mat_rs)
{
...
void TNNPlantID::detect(const cv::Mat &mat, types::ImageNetContent &content, unsigned int top_k)
{
  if (mat.empty()) return;
  ...
  std::shared_ptr<tnn::Mat> logits_mat; // (1,4066)
