...
#include "tnn_shufflenetv2.h"
#include "lite/utils.h"

using tnncv::TNNShuffleNetV2;

TNNShuffleNetV2::TNNShuffleNetV2(const std::string &_proto_path,
...
void TNNShuffleNetV2::transform(const cv::Mat &mat_rs)
{
...
void TNNShuffleNetV2::detect(const cv::Mat &mat, types::ImageNetContent &content, unsigned int top_k)
{
  if (mat.empty()) return;
  ...
  std::shared_ptr<tnn::Mat> logits_mat; // (1,1000)
  status = instance->GetOutputMat(logits_mat, cvt_param, "output", output_device_type);
