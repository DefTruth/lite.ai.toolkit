...
#include "tnn_tencent_cifp_face.h"

using tnncv::TNNTencentCifpFace;

TNNTencentCifpFace::TNNTencentCifpFace(const std::string &_proto_path,
...
void TNNTencentCifpFace::transform(const cv::Mat &mat_rs)
{
  // push into input_mat
...
void TNNTencentCifpFace::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  cv::Mat mat_rs;
