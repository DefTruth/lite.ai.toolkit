...
#include "tnn_tencent_curricular_face.h"

using tnncv::TNNTencentCurricularFace;

TNNTencentCurricularFace::TNNTencentCurricularFace(const std::string &_proto_path,
...
void TNNTencentCurricularFace::transform(const cv::Mat &mat_rs)
{
  // push into input_mat
...
void TNNTencentCurricularFace::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  cv::Mat mat_rs;
