...
void TNNYoloV5::resize_unscale(const cv::Mat &mat, cv::Mat &mat_rs,
...
void TNNYoloV5::transform(const cv::Mat &mat_rs)
{
  // push into input_mat
...
void TNNYoloV5::detect(const cv::Mat &mat, std::vector<types::Boxf> &detected_boxes,
...
void TNNYoloV5::generate_bboxes(const YoloV5ScaleParams &scale_params,
                                std::vector<types::Boxf> &bbox_collection,
                                ...
                                int img_width)
{
  ...
  status = _instance->GetOutputMat(pred_mat, pred_cvt_param, "pred", output_device_type);
  if (status != tnn::TNN_OK)
  {
#ifdef LITETNN_DEBUG
...
void TNNYoloV5::nms(std::vector<types::Boxf> &input, std::vector<types::Boxf> &output,
