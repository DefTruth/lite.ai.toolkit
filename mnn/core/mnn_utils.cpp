...
//*************************************** mnncv::utils **********************************************//
std::string mnncv::utils::to_string(const std::wstring &wstr)
{
...
std::wstring mnncv::utils::to_wstring(const std::string &str)
{
...
void mnncv::utils::draw_axis_inplace(cv::Mat &mat_inplace,
...
cv::Mat mnncv::utils::draw_axis(const cv::Mat &mat,
                                const types::EulerAngles &euler_angles,
                                float size, int thickness)
{
...
cv::Mat mnncv::utils::draw_landmarks(const cv::Mat &mat, types::Landmarks &landmarks)
{
...
void mnncv::utils::draw_landmarks_inplace(cv::Mat &mat, types::Landmarks &landmarks)
{
...
void mnncv::utils::draw_boxes_inplace(cv::Mat &mat_inplace, const std::vector<types::Boxf> &boxes)
{
...
cv::Mat mnncv::utils::draw_boxes(const cv::Mat &mat, const std::vector<types::Boxf> &boxes)
{
...
cv::Mat mnncv::utils::draw_age(const cv::Mat &mat, types::Age &age)
{
...
void mnncv::utils::draw_age_inplace(cv::Mat &mat_inplace, types::Age &age)
{
  if (!age.flag) return;
  const unsigned int offset = static_cast<unsigned int>(
...
cv::Mat mnncv::utils::draw_gender(const cv::Mat &mat, types::Gender &gender)
{
...
void mnncv::utils::draw_gender_inplace(cv::Mat &mat_inplace, types::Gender &gender)
{
...
cv::Mat mnncv::utils::draw_emotion(const cv::Mat &mat, types::Emotions &emotions)
{
...
void mnncv::utils::draw_emotion_inplace(cv::Mat &mat_inplace, types::Emotions &emotions)
{
...
void mnncv::utils::hard_nms(std::vector<types::Boxf> &input, std::vector<types::Boxf> &output,
...
void mnncv::utils::blending_nms(std::vector<types::Boxf> &input, std::vector<types::Boxf> &output,
                                float iou_threshold, unsigned int topk)
{
  if (input.empty()) return;
  std::sort(input.begin(), input.end(),
...
void mnncv::utils::offset_nms(std::vector<types::Boxf> &input, std::vector<types::Boxf> &output,
                              float iou_threshold, unsigned int topk)
{
  if (input.empty()) return;
  std::sort(input.begin(), input.end(),
