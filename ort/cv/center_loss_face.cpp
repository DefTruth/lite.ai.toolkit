...
#include "center_loss_face.h"
#include "ort/core/ort_utils.h"

using ortcv::CenterLossFace;

Ort::Value CenterLossFace::transform(const cv::Mat &mat)
{
...
void CenterLossFace::detect(const cv::Mat &mat, types::FaceContent &face_content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  ...
  auto embedding_dims = output_node_dims.at(0); // (1,2048)
  const unsigned int hidden_dim = embedding_dims.at(1); // 2048
