...
DeepLabV3ResNet101::DeepLabV3ResNet101(const std::string &_onnx_path, unsigned int _num_threads) :
...
void DeepLabV3ResNet101::print_debug_string()
{
...
ort::Value DeepLabV3ResNet101::transform(const cv::Mat &mat)
{
...
void DeepLabV3ResNet101::detect(const cv::Mat &mat, types::SegmentContent &content)
{
  if (mat.empty()) return;
  // 1. make input tensor
  ...
  for (unsigned int i = 0; i < output_height; ++i)
  {
    ...
    for (unsigned int j = 0; j < output_width; ++j)
    {
      ...
      // assign color for detected class.
